      ...
      content: (
        <div>
          <p>
            ...
            <a href="https://forms.gle/Yd7HxTMiU7DWht6R6" target='blank_' className="text-green-500 hover:underline inline-flex items-center">
  ...
  return (
    <div className="mt-3 d-flex flex-column p-2 container mb-5">
      ...
      <div className="d-flex d-lg-none justify-content-between align-items-center mb-3">
        <button aria-label='Anterior' onClick={goToPrevious} className="border-0 bg-transparent">
          <ChevronLeft />
        </button>
        <h3 className="fs-6 m-0">{guidelineSections[activeSection].title}</h3>
        <button aria-label='Siguiente' onClick={goToNext} className="border-0 bg-transparent">
          <ChevronRight />
        </button>
      </div>
        ...
        >
          {guidelineSections.map((section, index) => (
            <button
              ...
              className={`d-flex text-start h-100 align-items-center border-0 bg-transparent py-2 ${activeSection === index ? "fw-bold" : ""
                }`}
            >
